...
public class RuleAdministrator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        StartCoroutine(m_UIControlWorker.Run());
    }
            ...
            (RuleCheckWorker.OccuredActionState ActionEvent) =>
            {
                switch (ActionEvent)
                {
                    ...
                    case RuleCheckWorker.OccuredActionState.SkillCard:
                        {}
                        break;
                    case RuleCheckWorker.OccuredActionState.TurnActionEnd:
                        {}
                        break;
                    case RuleCheckWorker.OccuredActionState.PhaseEnded:
                        {
                            ++m_PhaseCount;
                            m_UIControlWorker.SetPhaseText(m_PhaseCount);
                        }
                        break;
                    default:
                        {}
    ...
    private void InitUIControlWorker()
    {
        m_UIControlWorker.RegisterSkillButtonListener((int selectedSkill) =>
        {
            switch(selectedSkill)
            {
                ...
                default:
                    {
                        Debug.Assert(false, "This Can't be Happen.");
                    }
                    break;
            }
        });

        m_UIControlWorker.RegisterBattleStyleChangeListener((bool isBattleAutomatic) =>
        {
            m_RuleCheckWorker.IsBattleAutomatic = isBattleAutomatic;
        });
    }
}
